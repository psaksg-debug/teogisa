...
export function ensureArticleMedia(value: string) {
  ...
  if (images.length === 0) {
    html = `<figure class="article-image"><img src="/article-thumbnail-sprite.png" alt="퇴직생활연구소 실전 무료 가이드 시각자료" loading="lazy" decoding="async"></figure>\n` + html;
  } else {
      ...
      if (alt.length < 8 || ["이미지", "본문 관련 이미지", "사진"].includes(alt)) {
        html = html.replace(/<img\b([^>]*)>/gi, (match) => {
          if (/alt\s*=\s*["'][^"']*["']/i.test(match)) {
            return match.replace(/alt\s*=\s*["'][^"']*["']/i, 'alt="퇴직생활연구소 실전 무료 가이드 시각자료"');
          }
          return match.replace(/<img\b/i, '<img alt="퇴직생활연구소 실전 무료 가이드 시각자료" ');
        });
      }
    }
  }
