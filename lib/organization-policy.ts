...
export const organizationNotice = {
  id: "organization-policy-2026-08-14-v1",
...
export const organizationPolicyRecipients:OrganizationPolicyRecipient[] = [
  ...
  ...allEditorialAuthors.map(member=>({id:`editorial:${member.id}`,name:member.name,title:member.role,department:"편집부",applied:true as const})),
  ...promotionTeam.map(member=>({id:`promotion:${member.id}`,name:member.name,title:member.title,department:"홍보부",applied:true as const})),
  ...
  ...managementDepartment.map(member=>({id:`management:${member.id}`,name:member.name,title:member.role,department:"관리부",applied:true as const})),
];
...
export const organizationPolicyCoverage = {
  ...
  departments: ["콘텐츠기획팀", "편집부", "품질디자인팀", "홍보부", "관리부"].map(department=>({department,count:organizationPolicyRecipients.filter(member=>member.department===department).length})),
};
