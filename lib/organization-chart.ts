...
export const ORGANIZATION_CHART_VERSION="2026.08.14-team-v2";
...
export const organizationChartDepartments:OrganizationChartDepartment[]=[
  {id:"content-planning",name:"콘텐츠기획팀",shortName:"기획",lead:"판",reportsTo:"대표",responsibility:"독자 질문과 사업목표를 제작 가능한 주제 포트폴리오와 브리프로 바꿉니다.",workflow:"독자 질문 → 주제 지도 → 제작 브리프 → 콘텐츠편집팀 인계",members:contentPlanningTeam.map(member=>({id:member.id,name:member.name,title:member.title,responsibility:member.mission}))},
  {id:"editorial",name:"콘텐츠편집팀",shortName:"편집",lead:"데스크",reportsTo:"대표",responsibility:"공식 출처를 확인하고 퇴직생활연구소만의 설명·사례·표와 행동 안내로 작성합니다.",workflow:"브리프 인수 → 취재·집필 → 출처·독창성 검토 → 품질검수 요청",members:allEditorialAuthors.map(member=>({id:member.id,name:member.name,title:member.role,responsibility:member.specialty}))},
  ...
  {id:"promotion",name:"홍보마케팅팀",shortName:"홍보",lead:"픽",reportsTo:"대표",responsibility:"승인된 공개 콘텐츠의 검색 노출과 외부채널 홍보안을 관리합니다.",workflow:"발행물 확인 → 채널별 홍보안 → 책임자 확인 → 실행·성과 기록",members:promotionTeam.map(member=>({id:member.id,name:member.name,title:member.title,responsibility:member.scope}))},
  {id:"management",name:"경영관리팀",shortName:"관리",lead:"강한결",reportsTo:"대표",responsibility:"전사 사규, 발행정책, 자동화, 리소스, 감사와 시정조치를 통제합니다.",workflow:"상시감시 → 전사감사 → 즉시보고 → 시정조치·재점검",members:managementDepartment.map(member=>({id:member.id,name:member.name,title:member.role,responsibility:member.mission}))},
];
