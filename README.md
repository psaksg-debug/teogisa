...
콘텐츠편집팀장은 `데스크`이고, 분야별 편집자는 `lib/editorial-team.ts`에 정의되어 있습니다(수익실험 `원`, 지원금·세무·노무 `가드`, 생활도구 `툴`, 지역정보 `로컬`, 건강·예방 `케어`, 영상 `큐`, 연금·보험 `노후`, 세금·보험 `절세`, 투자·재테크 `자산`, 생활경제 `살림`, 유용한 도구 `픽`).
