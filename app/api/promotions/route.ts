...
export async function POST(request:Request){const auth=await requireOwnerApi(request);if(auth.response)return auth.response;try{const payload=await request.json() as {action?:"prepare"|"execute";postId?:number;id?:number};if(payload.action==="prepare")return Response.json({campaign:await preparePromotionCampaign(payload.postId)},{status:201});if(payload.action==="execute"&&payload.id)return Response.json({campaign:await executePromotionCampaign(payload.id)});return Response.json({error:"지원하지 않는 홍보 작업입니다."},{status:400});}catch(error){return Response.json({error:error instanceof Error?error.message:"홍보 작업을 처리하지 못했습니다."},{status:500});}}
