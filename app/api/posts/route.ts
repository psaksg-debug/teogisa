...
export async function POST(request:Request){
  ...
  try{
    ...
    const body=appendSourceUrl(p.body,p.sourceUrl);
    const post=await createPost({title:p.title.trim(),slug:slugify(p.slug||p.title),excerpt:p.excerpt?.trim()||body.slice(0,120),body,category:p.category||"퇴직 준비",tags:(p.tags||"").split(",").map(x=>x.trim()).filter(Boolean),status,publishedAt:status==="published"?new Date().toISOString().slice(0,10):"",scheduledAt:status==="scheduled"?p.scheduledAt||null:null,readingMinutes:Math.max(1,Math.ceil(body.length/700)),visual:p.visual?.trim()||"NEW"});
