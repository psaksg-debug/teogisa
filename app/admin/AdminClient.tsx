...
export default function AdminClient({ username }: { username: string }) {
  ...
  // 가장 중요한 글 목록을 먼저 불러오고, 나머지는 순차적으로 불러옵니다.
  ...
  async function loadSecondaryData() {
    try {
      ...
      if (managementResponse.ok) {
        setManagementMembers(managementData.members); setManagementIssues(managementData.issues); setManagementRuns(managementData.runs);
        setOrganizationNotice(managementData.notice); setPolicyCoverage(managementData.policyCoverage); setPolicyRecipients(managementData.policyRecipients);
        setOriginalityChecks(managementData.originalityChecks); setCompanyRules(managementData.companyRules); setCompanyResources(managementData.companyResources);
      }
      if (auditResponse.ok) { setAuditScope(auditData.scope); setAuditOfficers(auditData.officers); setAuditDomains(auditData.domains); setAuditRuns(auditData.runs); setAuditFindings(auditData.findings); }
      if (activityResponse.ok) { setActivityPlans(activityData.plans); setActivityRuns(activityData.runs); }
    } catch (error) {
  ...
  async function finishPromotion(id: number) {
    setSaving(true); const response = await fetch("/api/promotions", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ action: "execute", id }) }); const data: any = await response.json(); setSaving(false); if (response.ok) { setMessage(`‘${data.campaign.title}’ 홍보 실행을 기록했습니다.`); await loadSecondaryData(); } else setMessage(data.error);
  }
      ...
      <div className="admin-dashboard">
        <section>
          ...
          <div className="panel">
            ...
            <div className="admin-list">{posts.map((post) => (
              <div className={`admin-item-wrapper ${selected?.id === post.id ? "selected" : ""}`} key={post.id}>
                ...
                {post.status === "published" && <Link href={`/posts/${post.slug}`} target="_blank" className="admin-item-link" title="새 탭에서 발행된 글 보기">🔗</Link>}
              </div>
            ))}</div>
          </div>
        </section>
