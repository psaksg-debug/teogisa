...
export const metadata:Metadata={title:"퇴직생활 수익화 프로젝트 소개",description:"퇴직 후 생활비, 지원제도, 새로운 일과 건강을 내 상황에 맞는 실행 순서로 바꾸는 퇴직생활 수익화 프로젝트를 소개합니다.",alternates:{canonical:"/about"}};
...
export default function About(){return <><InnerHeader path="/about" eyebrow="ABOUT THE PROJECT" title="퇴직 후의 선택이 조금 덜 막막해지도록." description="생활비부터 새 수입과 건강까지, 흩어진 정보를 오늘 할 수 있는 순서로 바꾸는 퇴직생활 실전 가이드입니다."/><main className="content-shell article-copy policy-copy">
  <h2>퇴직하면 가장 먼저 무엇을 해야 할까요?</h2><p>정답은 사람마다 다르지만 시작점은 비슷합니다. 한 달에 꼭 필요한 돈을 알고, 지금 가진 자금으로 버틸 시간을 계산하고, 놓치면 안 되는 제도와 날짜를 챙기는 일입니다. 퇴직생활 수익화 프로젝트는 이 세 가지를 정리한 뒤 새로운 일과 수입을 준비하도록 안내합니다.</p>
