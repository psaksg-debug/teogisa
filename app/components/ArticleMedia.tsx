...
export function ArticleSupport({ post }: { post: Post }) {
  ...
  return <>
    {enrichment.officialLinks.length > 0 && <section className="article-reference-grid" aria-label="공식자료와 추가 확인처">
    ...
    {enrichment.images?.filter(image=>!post.body.includes(image.src)).map(image=><figure className="article-image" key={image.src}><img src={image.src} alt={image.alt} width={image.width} height={image.height} loading="lazy" decoding="async"/><figcaption>{image.caption}</figcaption></figure>)}
